...
/**
 * @brief Abstract base class for GPU-batched robust loss (robustifier)
 * functions.
 *
 * A loss function modifies the squared residual to reduce the influence of
 * outliers. Given squared error s, it computes a triplet rho = (rho(s),
 * rho'(s), rho''(s)) stored as a float3, where:
...
public:
  /**
   * @brief Evaluates the loss function for a batch of squared residuals on the
   * GPU.
   *
   * @param s          Device pointer to an array of squared residuals
   * (input/output).
