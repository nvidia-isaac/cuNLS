...
void LaunchFusedSo2Plus(cudaStream_t stream, const float *x, const float *delta,
  ...
  fused_so2_plus_kernel<<<grid, kBlockSize, 0, stream>>>(x, delta, result, n,
                                                         negate_delta);
...
SO2StateBatch::SO2StateBatch(cuBLASHandle &cublas_handle,
                             const float *device_ptr, size_t num_blocks)
    : Base(device_ptr, num_blocks), cublas_handle_(cublas_handle),
      delta_rotations_(num_blocks), angles_(num_blocks) {}
