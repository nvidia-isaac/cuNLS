...
/** Row-major 4x4 multiply: C = A * B per batch element (64 FMAs, fully
 * unrolled). */
...
SL4StateBatch::SL4StateBatch(cuBLASHandle &cublas_handle,
                             const float *device_ptr, size_t num_blocks)
    : Base(device_ptr, num_blocks), cublas_handle_(cublas_handle),
      delta_transforms_(num_blocks), twists_(num_blocks * 15) {}
...
void SL4StateBatch::Plus(const float *x, const float *delta,
  ...
  sl4_matmul_kernel<<<grid, kBlockSize, 0, stream>>>(
      x, reinterpret_cast<const float *>(delta_transforms_.data()),
      x_plus_delta, n);
