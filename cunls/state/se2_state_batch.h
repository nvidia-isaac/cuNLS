 ...
 *                              [    0,           0,       1]]
 * where [tx, ty] = V * [v_x, v_y] with V = [[sin(theta)/theta, -(1-cos(theta))/theta],
...
class SE2StateBatch
 ...
 public:
   ...
   * @param device_ptr Pointer to GPU device memory containing the SE(2) transforms.
  ...
  /**
   * @brief Constructs a batch of SE(2) state blocks with constant state constraints.
   *
   * @param cublas_handle Reference to an externally-owned cuBLAS handle.
   * @param device_ptr Pointer to GPU device memory containing the SE(2) transforms.
   *                   Must point to at least num_blocks * 9 floats of allocated memory.
 ...
 private:
   ...
   * using the SE(2) exponential map, then performs batched matrix multiplication
   * using cuBLAS.
   *
