...
// small-angle)
__device__ __forceinline__ void rodrigues_row(const float *phi, int r, float k1, float k2, float k3,
                                              float k4, float *row3, float tol = 1e-5f) {
...
#pragma unroll
  ...
  if (active) {
    if (row < 3) {
      ...
      float tmp[3];
#pragma unroll
      for (int j = 0; j < 3; j++) tmp[j] = Jr[0] * s_Q[j] + Jr[1] * s_Q[3 + j] + Jr[2] * s_Q[6 + j];
