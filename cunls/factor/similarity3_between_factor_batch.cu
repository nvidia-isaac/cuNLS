...
// Duplicates the logic from jacobian_right_inverse_sim3_kernel to allow
// inlining.
...
#pragma unroll
    for (int c = 0; c < 3; ++c)
      W2[r * 3 + c] =
          W[r * 3] * W[c] + W[r * 3 + 1] * W[3 + c] + W[r * 3 + 2] * W[6 + c];
...
#pragma unroll
    for (int c = 0; c < 3; ++c) {
      UW[r * 3 + c] = U_mat[r * 3] * W[c] + U_mat[r * 3 + 1] * W[3 + c] +
                      U_mat[r * 3 + 2] * W[6 + c];
      UW2[r * 3 + c] = U_mat[r * 3] * W2[c] + U_mat[r * 3 + 1] * W2[3 + c] +
                       U_mat[r * 3 + 2] * W2[6 + c];
    }
  ...
  constexpr float c1 = 0.5f, c2 = 1.f / 12.f, c4 = -1.f / 720.f,
                  c6 = 1.f / 30240.f;
  constexpr float d0v = 0.5f, d1v = 1.f / 12.f, d3v = -1.f / 720.f,
                  d5v = 1.f / 30240.f;
...
#pragma unroll
  ...
  {
    float na = lam * ma, nb2 = lam * mb + ma - theta2 * mc, nc = lam * mc + mb;
    ma = na;
    mb = nb2;
    mc = nc;
  }
...
#pragma unroll
    for (int r = 0; r < 3; ++r)
      for (int c = 0; c < 3; ++c)
        WB[r * 3 + c] = W[r * 3] * Bn[c] + W[r * 3 + 1] * Bn[3 + c] +
                        W[r * 3 + 2] * Bn[6 + c];
#pragma unroll
    for (int i = 0; i < 9; ++i)
      Bn[i] = UW[i] + lam * Bn[i] + WB[i];
  }
...
// Left block (cols 0-6):  H_left  = -J_l^{-1}(r) * Ad(Delta) = -J_r^{-1}(-r) *
// Ad Right block (cols 7-13): H_right =  J_r^{-1}(r) Eliminates 6 separate
// kernel launches + cuBLAS GEMM.
__global__ void __launch_bounds__(128, 2) sim3_between_fused_jacobians_kernel(
...
Similarity3BetweenFactorBatch::Similarity3BetweenFactorBatch(
    ...
    : pose_deltas_ptr_(pose_deltas_ptr), num_factors_(num_factors),
      cublas_handle_(cublas_handle), poses_left_(num_factors),
...
bool Similarity3BetweenFactorBatch::Evaluate(float *residuals, float *jacobians,
  ...
  if (jacobians != nullptr) {
    size_t fused_blocks =
        (num_factors + kSim3FusedBlockSize - 1) / kSim3FusedBlockSize;
