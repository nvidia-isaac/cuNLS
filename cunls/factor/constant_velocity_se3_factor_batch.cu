...
/**
 * @brief Gathers pose_k, pose_{k+1} from state_pointers and computes
 *        pose_rel = pose_k^{-1} * pose_{k+1}, exploiting the SE(3)
 *        [0 0 0 1] last row (same technique as the SE(3) between factor).
 */
__global__ void cv_se3_relative_pose_kernel(float const *const *state_pointers, size_t num_factors,
...
/**
 * @brief Assembles residuals and (optionally) the 12x24 dense Jacobian block
 *        from the precomputed twist/Jl_inv/Jr_inv scratch buffers.
 *
...
__global__ void cv_se3_assemble_kernel(float const *const *state_pointers, const Vector<6> *twist,
  ...
  const float *__restrict__ vel_k = state_pointers[4 * tid + 2];
  const float *__restrict__ vel_k1 = state_pointers[4 * tid + 3];
  const float *__restrict__ tw = twist[tid].data();
  const float *__restrict__ jli = jl_inv[tid].data();
  const float dt_i = dt[tid];

  float *__restrict__ res = residuals + tid * 2 * kDim;
#pragma unroll
  for (int i = 0; i < kDim; i++) {
    res[i] = tw[i] - dt_i * vel_k[i];
    float acc = -vel_k[i];
#pragma unroll
    for (int j = 0; j < kDim; j++) acc += jli[i * kDim + j] * vel_k1[j];
    res[kDim + i] = acc;
  }
  ...
  float *__restrict__ jac = jacobians + tid * 2 * kDim * kCols;
...
#pragma unroll
    for (int j = 0; j < kDim; j++) {
      ...
      row_pose[2 * kDim + j] = (i == j) ? -dt_i : 0.0f;  // d(r_pose)/d(vel_k)
      row_pose[3 * kDim + j] = 0.0f;                     // d(r_pose)/d(vel_k+1)

      row_vel[j] = 0.0f;                                // d(r_vel)/d(pose_k)   (simplified)
      row_vel[kDim + j] = 0.0f;                         // d(r_vel)/d(pose_k+1) (simplified)
      row_vel[2 * kDim + j] = (i == j) ? -1.0f : 0.0f;  // d(r_vel)/d(vel_k)
      row_vel[3 * kDim + j] = jli[i * kDim + j];        // d(r_vel)/d(vel_k+1)
    }
  }
}

ConstantVelocitySE3FactorBatch::ConstantVelocitySE3FactorBatch(const float *dt_ptr,
                                                               size_t num_factors)
    : dt_ptr_(dt_ptr),
...
bool ConstantVelocitySE3FactorBatch::Evaluate(float *residuals, float *jacobians,
                                              float const *const *state_pointers,
                                              cudaStream_t stream) const {
  const size_t num_blocks = (num_factors_ + kBlockSize - 1) / kBlockSize;

  cv_se3_relative_pose_kernel<<<num_blocks, kBlockSize, 0, stream>>>(state_pointers, num_factors_,
                                                                     pose_rel_.data());
  ...
  cv_se3_assemble_kernel<<<num_blocks, kBlockSize, 0, stream>>>(
