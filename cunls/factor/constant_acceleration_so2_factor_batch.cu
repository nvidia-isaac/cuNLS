...
/**
 * @brief Assembles residuals and (optionally) the 3x6 Jacobian block. SO(2)
...
__global__ void ca_so2_assemble_kernel(float const *const *state_pointers, const float *twist,
  ...
  res[0] = tw - dt_i * vel_k - half_dt2 * accel_k;
  res[1] = vel_k1 - vel_k - dt_i * accel_k;
  res[2] = accel_k1 - accel_k;

  if (jacobians == nullptr) return;

  // Columns: [pose_k, pose_k+1, vel_k, vel_k+1, accel_k, accel_k+1]
  float *__restrict__ jac = jacobians + tid * 3 * 6;
  jac[0] = -1.0f;
  jac[1] = 1.0f;
  jac[2] = -dt_i;
  jac[3] = 0.0f;
