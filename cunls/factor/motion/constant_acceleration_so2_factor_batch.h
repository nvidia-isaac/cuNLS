 ...
 * SO(2). SO(2) is abelian, so `J_l = J_r = 1` and the residual/Jacobian
 * reduce to scalar arithmetic (`theta := Log(pose_k^{-1} * pose_{k+1})`):
 *
 *     r_pose  = theta - dt * vel_k - 0.5 * dt^2 * accel_k
 *     r_vel   = vel_{k+1} - vel_k - dt * accel_k
 *     r_accel = accel_{k+1} - accel_k
 *
 * Inherits from SizedFactorBatch<3, 1, 1, 1, 1, 1, 1>.
 */
