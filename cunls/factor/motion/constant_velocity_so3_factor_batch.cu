...
constexpr size_t kBlockSizeSO3CV = 256;

/** @brief Computes pose_rel = R_k^T * R_{k+1} for each factor (3x3). */
__global__ void cv_so3_relative_pose_kernel(float const *const *state_pointers, size_t num_factors,
                                            Matrix<3> *pose_rel) {
...
#pragma unroll
    for (int j = 0; j < 3; j++) {
      float acc = 0.0f;
#pragma unroll
      for (int k = 0; k < 3; k++) acc += L[k * 3 + i] * R[k * 3 + j];  // L^T * R
      out[i * 3 + j] = acc;
    }
}

/** @brief Assembles residuals and (optionally) the 6x12 Jacobian block. */
__global__ void cv_so3_assemble_kernel(float const *const *state_pointers, const Vector<3> *twist,
...
#pragma unroll
    for (int j = 0; j < kDim; j++) {
      ...
      row_pose[2 * kDim + j] = (i == j) ? -dt_i : 0.0f;
      row_pose[3 * kDim + j] = 0.0f;

      row_vel[j] = 0.0f;
      row_vel[kDim + j] = 0.0f;
      row_vel[2 * kDim + j] = (i == j) ? -1.0f : 0.0f;
      row_vel[3 * kDim + j] = jli[i * kDim + j];
    }
  }
}

ConstantVelocitySO3FactorBatch::ConstantVelocitySO3FactorBatch(const float *dt_ptr,
                                                               size_t num_factors)
    : dt_ptr_(dt_ptr),
...
bool ConstantVelocitySO3FactorBatch::Evaluate(float *residuals, float *jacobians,
                                              float const *const *state_pointers,
                                              cudaStream_t stream) const {
  const size_t num_blocks = (num_factors_ + kBlockSizeSO3CV - 1) / kBlockSizeSO3CV;
  ...
  cv_so3_assemble_kernel<<<num_blocks, kBlockSizeSO3CV, 0, stream>>>(
