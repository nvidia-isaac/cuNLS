...
constexpr size_t kBlockSizeSE2CV = 256;

/** @brief Computes pose_rel = T_k^{-1} * T_{k+1} for each factor (3x3, SE(2)). */
__global__ void cv_se2_relative_pose_kernel(float const *const *state_pointers, size_t num_factors,
                                            Matrix<3> *pose_rel) {
...
__global__ void cv_se2_assemble_kernel(float const *const *state_pointers, const Vector<3> *twist,
...
bool ConstantVelocitySE2FactorBatch::Evaluate(float *residuals, float *jacobians,
  ...
  const size_t num_blocks = (num_factors_ + kBlockSizeSE2CV - 1) / kBlockSizeSE2CV;
