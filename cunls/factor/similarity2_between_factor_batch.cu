 ...
 */
__global__ void
collect_sim2_left_poses_kernel(float const *const *state_pointers,
                               size_t num_factors, Matrix<3> *pose_left) {
  const int tid = threadIdx.x + blockIdx.x * blockDim.x;
  if (tid >= (int)num_factors)
    return;
  pose_left[tid] =
      *reinterpret_cast<const Matrix<3> *>(state_pointers[2 * tid]);
}
 ...
 *         [0 0, 0, 1]]
 */
__global__ void __launch_bounds__(256, 6) sim2_between_jacobian_kernel(
...
#pragma unroll
  for (int i = 0; i < 4; ++i) {
    float l0 = Jl[i * 4], l1 = Jl[i * 4 + 1], l2 = Jl[i * 4 + 2],
          l3 = Jl[i * 4 + 3];
...
Similarity2BetweenFactorBatch::Similarity2BetweenFactorBatch(
    ...
    : pose_deltas_ptr_(pose_deltas_ptr), num_factors_(num_factors),
      poses_left_(num_factors), poses_right_(num_factors),
...
bool Similarity2BetweenFactorBatch::Evaluate(float *residuals, float *jacobians,
  ...
  ComputeLogSim2(stream, reinterpret_cast<const float *>(poses_left_.data()),
