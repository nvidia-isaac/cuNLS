 ...
 */
#define THROW_ON_CUDA_ERROR(status) CHECK_CUDA_ERROR(status, cudaGetErrorString, true)

/**
 * @brief Logs a warning on CUDA runtime API errors without throwing.
 * @param status cudaError_t value returned by a CUDA runtime call.
 */
#define WARN_ON_CUDA_ERROR(status) CHECK_CUDA_ERROR(status, cudaGetErrorString, false)
}  // namespace cunls
