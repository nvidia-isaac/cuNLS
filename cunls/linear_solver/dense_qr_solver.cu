...
bool DenseQRSolver::Solve(cudaStream_t stream,
  ...
  THROW_ON_CUSOLVER_ERROR(cusolverDnSgeqrf(
      cusolver, n, n, dense_matrix_.data(), n, tau_.data(),
  ...
  THROW_ON_CUSOLVER_ERROR(cusolverDnSormqr(
      cusolver, CUBLAS_SIDE_LEFT, CUBLAS_OP_T,
      n, 1, n,
      dense_matrix_.data(), n, tau_.data(),
      rhs_copy_.data(), n,
      workspace_.data(), static_cast<int>(workspace_.size()),
      dev_info_.data()));
  ...
  THROW_ON_CUBLAS_ERROR(cublasStrsm(
      cublas, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_UPPER,
...
void DenseQRSolver::EnsureBuffersSize(cudaStream_t stream, size_t n) {
  ...
  if (n != last_n_ && n > 0) {
    ...
    THROW_ON_CUSOLVER_ERROR(cusolverDnSgeqrf_bufferSize(
        ...
        dense_matrix_.data(), static_cast<int>(n), &geqrf_lwork));

    int ormqr_lwork = 0;
    THROW_ON_CUSOLVER_ERROR(cusolverDnSormqr_bufferSize(
        handle, CUBLAS_SIDE_LEFT, CUBLAS_OP_T,
        static_cast<int>(n), 1, static_cast<int>(n),
        dense_matrix_.data(), static_cast<int>(n), tau_.data(),
        rhs_copy_.data(), static_cast<int>(n), &ormqr_lwork));
