 ...
 * ResidualBatch::Evaluate call.
 *
 * Layout: `num_residuals` consecutive `float` squared norms, then padding to
 * `alignof(float3)`, then `num_residuals` consecutive `float3` values (rho
...
/**
 * @brief Same scratch as `ResidualBatchWorkspaceSizeBytes`, expressed in
 * `float` elements (rounded up).
 *
 * Use this when sub-allocating inside a larger `float` arena (e.g. GaussNewton
 * `buffer_`).
 */
...
public:
  ...
  /**
   * @brief Evaluates residuals, Jacobians, and/or cost for the given state
   * pointers.
   *
