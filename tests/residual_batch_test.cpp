 ...
 * @brief Unit tests for ResidualBatch evaluation with Huber loss
 * robustification.
 *
 * Tests residual computation, cost evaluation, and Jacobian scaling using
 * PriorVectorFactorBatch with HuberLossFunctionBatch across multiple vector
 * sizes.
 */
...
public:
  ...
  void SetUp() override {
    ...
    robust_workspace_.resize(ResidualBatchWorkspaceNumFloats(num_vectors_));
...
/** @brief Tests that robustified residuals match the expected Huber-scaled
 * values. */
...
TYPED_TEST(ResidualBatchTest, Cost) {
  ...
  for (size_t i = 0; i < this->num_vectors_; i++) {
    ...
    // so can differ from host residual*residual*dim due to float order (worse
    // for large dim).
    ASSERT_NEAR(host_cost[i], expected_cost, 2e-4f);
  }
}

/** @brief Tests that robustified Jacobians match the expected Huber-corrected
 * values. */
