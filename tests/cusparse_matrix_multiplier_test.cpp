 ...
 * implementation. */
TEST(cuSPARSESparseMatrixMultiplierTest, ComputeHessian) {
  ...
  cuSPARSESparseMatrixMultiplier smm;
  CudaStream stream;
  Problem problem;

  smm.Initialize(stream.GetStream(), problem, input_matrix, hessian);
  THROW_ON_CUDA_ERROR(cudaStreamSynchronize(stream.GetStream()));

  {
    // Warm up
    smm.ComputeSquaredMatrix(stream.GetStream(), problem, input_matrix,
                             hessian);
    THROW_ON_CUDA_ERROR(cudaStreamSynchronize(stream.GetStream()));
  }
