 ...
 * @brief Unified SL(4) manifold tests: state batch dimensions, prior and
...
dvector<SL4Transform> GenerateRandomSL4(size_t n, uint32_t seed, float magnitude) {
  ...
  CudaStream stream;
  ComputeExpSL4(stream.GetStream(), reinterpret_cast<const float *>(xi_dev.data()), 15, 4, 16, n,
                reinterpret_cast<float *>(out.data()));
  THROW_ON_CUDA_ERROR(cudaStreamSynchronize(stream.GetStream()));
  return out;
}

dvector<SL4Transform> PerturbSL4(const dvector<SL4Transform> &targets, size_t n, uint32_t seed,
                                 float magnitude, cuBLASHandle &cublas) {
  dvector<SL4Transform> perturbations = GenerateRandomSL4(n, seed, magnitude);
...
TEST(SL4ManifoldTest, BetweenLMConvergence) {
  ...
  for (size_t i = 0; i < kN; ++i) {
    for (size_t j = 0; j < 16; ++j) {
      ASSERT_NEAR(opt_left[i][j], opt_right[i][j], 0.15f) << "transform " << i << ", element " << j;
    }
  }
}
