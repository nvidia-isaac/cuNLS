...
/**
 * @file similarity2_prior_factor_test.cpp
 * @brief Tests for Similarity2StateBatch and
 * Similarity2PriorFactorBatch with Levenberg-Marquardt convergence.
 *
 * Validates that Sim(2) state blocks have correct dimensions and that
 * perturbed Sim(2) transforms can be optimized to match targets using the
 * Similarity2 prior factor and LM minimizer.
 */
...
/**
 * @brief Helper to build a 3x3 Sim(2) matrix from (x, y, theta, scale).
 *
 * Constructs the row-major matrix:
...
/**
 * @brief Test fixture for Similarity2StateBatch and
 * Similarity2PriorFactorBatch.
 *
 * Creates target Sim(2) transformation matrices from random
 * (x, y, theta, scale) state and perturbed initial transforms, then
 * verifies dimension properties and that LM optimization converges.
 */
class Similarity2PriorCostTest : public ::testing::Test {
 public:
  void SetUp() override {
    ...
    std::uniform_real_distribution<float> perturbation_scale_dist(0.85f, 1.15f);
    ...
    for (size_t i = 0; i < num_transforms_; i++) {
      ...
      float target_scale = scale_dist(rng);
      ...
      float perturbed_scale = target_scale * perturbation_scale_dist(rng);

      target_transforms_[i] =
          MakeSim2Matrix(target_x, target_y, target_theta, target_scale);
      initial_transforms_[i] = MakeSim2Matrix(
          perturbed_x, perturbed_y, perturbed_theta, perturbed_scale);
    }
  }
  ...
  profiler::Domain profiler_domain_{"Similarity2PriorCostTest"};
};

/**
 * @brief Tests that TangentSize returns the correct dimension (4).
 *
 * The tangent space of Sim(2) has dimension 4: 2 for translation, 1 for
 * rotation, and 1 for log-scale.
 */
TEST_F(Similarity2PriorCostTest, TangentSize) {
  dvector<Matrix<3>> transforms_device(initial_transforms_);
  const float* data_ptr =
      reinterpret_cast<const float*>(transforms_device.data());
  Similarity2StateBatch sim2_states(cublas_handle_, data_ptr, num_transforms_);
  ASSERT_EQ(sim2_states.TangentSize(), 4);
}

/**
 * @brief Tests that AmbientSize returns the correct dimension (9).
 *
 * The ambient space is a 3x3 transformation matrix,
 * stored as 9 floats in row-major order.
 */
TEST_F(Similarity2PriorCostTest, AmbientSize) {
  dvector<Matrix<3>> transforms_device(initial_transforms_);
  const float* data_ptr =
      reinterpret_cast<const float*>(transforms_device.data());
  Similarity2StateBatch sim2_states(cublas_handle_, data_ptr, num_transforms_);
  ASSERT_EQ(sim2_states.AmbientSize(), 9);
}

/**
 * @brief Tests that NumStateBlocks returns the correct count.
 */
TEST_F(Similarity2PriorCostTest, NumStateBlocks) {
  dvector<Matrix<3>> transforms_device(initial_transforms_);
  const float* data_ptr =
      reinterpret_cast<const float*>(transforms_device.data());
  Similarity2StateBatch sim2_states(cublas_handle_, data_ptr, num_transforms_);
  ASSERT_EQ(sim2_states.NumStateBlocks(), num_transforms_);
}

/**
 * @brief Tests that LM optimization converges for Sim(2) prior cost.
 *
 * Each Sim(2) transform is initialized with a perturbed pose/scale and
 * optimized to match a target transform using Similarity2PriorFactorBatch.
 * Minimizes Log(T_target^{-1} * T_current) over the Sim(2) manifold.
 */
TEST_F(Similarity2PriorCostTest, LMConvergence) {
  ...
  // Create Sim(2) state batch
  const float* state_data_ptr =
      reinterpret_cast<const float*>(transforms_device.data());
  Similarity2StateBatch state_batch(cublas_handle_, state_data_ptr, num_transforms_);

  // Create Sim(2) prior factor batch
  Similarity2PriorFactorBatch factor_batch(cublas_handle_, targets_device.data(),
                                                  num_transforms_);
