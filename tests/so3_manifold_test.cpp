 ...
 * @brief Unified SO(3) manifold tests: state batch dimensions, prior and between
...
dvector<Matrix<3>> GenerateRandomSO3(size_t n, uint32_t seed, float magnitude) {
  ...
  ComputeExpSO3(stream.GetStream(),
                ...
                3, 3, 9, n, reinterpret_cast<float*>(out.data()));
...
TEST(SO3ManifoldTest, PriorLMConvergence) {
  ...
  SO3PriorFactorBatch factor_batch(cublas, targets.data(), kN);
...
TEST(SO3ManifoldTest, BetweenLMConvergence) {
  ...
  SO3BetweenFactorBatch factor_batch(cublas, deltas_dev.data(), kN);
