...
dvector<Matrix<3>> GenerateRandomSO3(size_t n, uint32_t seed, float magnitude) {
  ...
  CudaStream stream;
  ComputeExpSO3(stream.GetStream(), reinterpret_cast<const float *>(twists_dev.data()), 3, 3, 9, n,
                reinterpret_cast<float *>(out.data()));
  THROW_ON_CUDA_ERROR(cudaStreamSynchronize(stream.GetStream()));
  return out;
}

dvector<Matrix<3>> PerturbSO3(const dvector<Matrix<3>> &targets, size_t n, uint32_t seed,
  ...
  THROW_ON_CUBLAS_ERROR(
      cublasSgemmStridedBatched(handle, CUBLAS_OP_N, CUBLAS_OP_N, 3, 3, 3, &alpha,
