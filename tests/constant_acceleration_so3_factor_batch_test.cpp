...
/**
 * @file constant_acceleration_so3_factor_batch_test.cpp
 * @brief Unit tests for ConstantAccelerationSO3FactorBatch.
 */
...
Matrix<3> ExpSO3(const Vector<3> &twist) {
  ...
  ComputeExpSO3(stream.GetStream(), reinterpret_cast<const float *>(twist_dev.data()), 3, 3, 9, 1,
...
Matrix<3> JacobianLeftSO3(const Vector<3> &twist) {
  ...
  dvector<Vector<3>> twist_dev({twist});
  dvector<Matrix<3>> out_dev(1);
  ComputeJacobianLeftSO3(stream.GetStream(), reinterpret_cast<const float *>(twist_dev.data()), 3,
                         3, 9, 1, reinterpret_cast<float *>(out_dev.data()));
  THROW_ON_CUDA_ERROR(cudaStreamSynchronize(stream.GetStream()));
  Matrix<3> out;
  out_dev.CopyToHost(&out, 1);
  return out;
...
class ConstantAccelerationSO3FactorBatchTest : public ::testing::Test {
 protected:
  std::mt19937 rng_{143};
};

TEST_F(ConstantAccelerationSO3FactorBatchTest, ZeroResidualForExactConstantAcceleration) {
  ...
  const float dt = 0.22f;

  Matrix<3> pose_k = ExpSO3({0.1f, -0.15f, 0.05f});
  Vector<3> step_twist;
  ...
  Vector<3> vel_plus_dt_accel;
  for (int i = 0; i < 3; i++) vel_plus_dt_accel[i] = vel_k[i] + dt * accel_k[i];
  Vector<3> vel_k1 = MatVec3(jl, vel_plus_dt_accel);
  Vector<3> accel_k1 = MatVec3(jl, accel_k);
...
TEST_F(ConstantAccelerationSO3FactorBatchTest, NumericalJacobianVelAccelBlocks) {
