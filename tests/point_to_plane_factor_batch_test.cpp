...
/**
 * @file point_to_plane_factor_batch_test.cpp
 * @brief Unit tests for PointToPlaneFactorBatch.
 *
 * Tests the point-to-plane factor by:
...
/**
 * @brief Test fixture for PointToPlaneFactorBatch unit tests.
 *
...
class PointToPlaneFactorBatchTest : public ::testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    // Generate random unit normals (Nq)
 ...
 *
 * When T = I, residual = Nq . (p - q) for each correspondence.
 */
TEST_F(PointToPlaneFactorBatchTest, ResidualIdentity) {
  ...
  for (size_t i = 0; i < num_correspondences_; i++) {
    ...
    float expected = Dot3(nq_normals_[i], diff);
    EXPECT_NEAR(host_residuals[i], expected, 1e-5f)
 ...
 *
 * Since p = T @ q by construction, (p - T@q) = 0 and therefore
 * Nq . (p - T@q) = 0 for any normal.
 */
TEST_F(PointToPlaneFactorBatchTest, ResidualGroundTruth) {
 ...
 *
 * When T = I (R = I, t = 0), n' = R^T * Nq = Nq, so:
 *   - d(r)/d(omega) = Nq^T * [q]_x  (1x3)
 *   - d(r)/d(rho)   = -Nq^T          (1x3)
 */
TEST_F(PointToPlaneFactorBatchTest, JacobianIdentity) {
  ...
  for (size_t i = 0; i < num_correspondences_; i++) {
    ...
    //   col 0: n[1]*q[2] - n[2]*q[1]
    //   col 1: n[2]*q[0] - n[0]*q[2]
    //   col 2: n[0]*q[1] - n[1]*q[0]
    EXPECT_NEAR(jac[0], n[1] * q[2] - n[2] * q[1], 1e-5f)
        << "Rotation Jacobian col 0 at " << i;
    EXPECT_NEAR(jac[1], n[2] * q[0] - n[0] * q[2], 1e-5f)
        << "Rotation Jacobian col 1 at " << i;
    EXPECT_NEAR(jac[2], n[0] * q[1] - n[1] * q[0], 1e-5f)
 ...
 */
TEST_F(PointToPlaneFactorBatchTest, EvaluateWithoutJacobians) {
  auto test_range =
 ...
 * pose T_init and optimizes to minimize sum of (Nq . (p - T @ q))^2.
 *
 * Note: Point-to-plane with random normals provides 6 DOF constraints
 * when there are enough correspondences with varied normals, so the
 * full SE(3) pose should be recoverable.
 */
TEST_F(PointToPlaneFactorBatchTest, OptimizeDisturbedPose) {
